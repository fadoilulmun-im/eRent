...
export default (props) => {
  ...
  useEffect(()=>{
    ...
    console.log("update imag",props)
  },[props.src])
  return (
    <>
      {isError ? (
        
          <svg width={props.style?(props.style.maxWidth||'3rem'):'3rem'} height={props.style?(props.style.maxHeight||'3rem'):'3rem'} xmlns="http://www.w3.org/2000/svg" data-name="Layer 1"
          viewBox="0 0 24 24"><path fill="#9CA3AF" 
      ...
      ) : <img src={props.src?props.src:''} onError={()=>{setIsError(true);console.log("error",props.src)}} className={props.className||""} style={props.style||{}}/>}
    </>
  )
}